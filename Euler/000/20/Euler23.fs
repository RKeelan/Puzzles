...
let main argv =
    ...
    let numbers = seq {1 .. 28124} |> Seq.filter isNotAbundantSum
    let sum = numbers |> Seq.sum
    printfn $"The sum of all the positive integers which cannot be written as the sum of two abundant numbers is {sum}"
